...
>
  <div
    class="mx-auto h-10 w-px bg-gradient-to-b from-transparent via-white/20 to-transparent"
  ></div>
  {#each SOCIAL_LINKS as s (s.id)}
    ...
    >
      <Icon
        icon={iconNameById[s.id] ?? "simple-icons:linktree"}
        width="22"
        height="22"
        class="text-brand-500 opacity-90 group-hover:opacity-100"
        aria-hidden="true"
      />
      <span
        class="pointer-events-none absolute left-12 whitespace-nowrap px-2 py-1 rounded-lg text-xs bg-neutral-900/90 border border-white/10 opacity-0 group-hover:opacity-100 translate-x-1 group-hover:translate-x-0 transition-all"
      >
        {s.label}
      </span>
    </a>
...
>
  <div class="mx-auto max-w-screen-xl px-3 pb-2">
    <div
      class="rounded-2xl bg-neutral-900/80 backdrop-blur border border-white/10 shadow-2xl"
    >
